...
export default function SvgPuncher() {
    ...
    const traceImage = useCallback(() => {
        ...
        if (paper.project) {
            // CRITICAL BUG FIX: Explicitly remove all existing items to prevent stacking when threshold updates.
            ...
            paper.project.importSVG(svgStr, {
                onLoad: (item: any) => {
                    ...
                    if (unifiedItem) {
                        ...
                        unifiedItem.name = 'puppet_base_' + Math.random().toString(36).substr(2, 5);
                    ...
                    paper.view.update(); // Force immediate update
        ...
        interactTool.onMouseDown = (event: any) => {
            ...
            if (tool === 'select') {
                ...
                if (hit && hit.item && hit.item.data.isPuppetPart) {
                    ...
                    result.name = 'joint_' + Math.random().toString(36).substr(2, 5);
                ...
                if (hit && hit.item && hit.item.data.isPuppetPart) {
                    ...
                    result.data.isPuppetPart = true;
                    result.name = 'hole_' + Math.random().toString(36).substr(2, 5);
                    hit.item.replaceWith(result);
                    hole.remove();
                }
            } else if (tool === 'erase') {
                        ...
                        try {
                            ...
                            const finalize = (p: any, prefix: string) => {
                                ...
                                if (p instanceof paper.CompoundPath) {
                                    ...
                                    p.name = prefix + '_' + Math.random().toString(36).substr(2, 5);
                                    puppetLayer.addChild(p);
                                }
                            };
                            finalize(male, 'joint');
                            finalize(female, 'puppet');
            ...
            <div className="w-full md:w-72 bg-white p-6 flex flex-col gap-6 border-r border-slate-200 shrink-0 shadow-sm overflow-y-auto styled-scrollbar">
                        ...
                        if (paper.project) { 
                            ...
                            try {
                                ...
                                await exportSvgTo3mf(svg, {
                                    baseLayerId: 'puppetLayer', // Doesn't matter as much now that isDiscreteMode is true
                                    baseDepth: 2,
                                    itemDepth: 2,
                                ...
                                const event = new CustomEvent(EVENTS.EXPORT_3MF_SUCCESS);
                ...
                >
                    {isProcessing ? (
                        ...
                        <><Download className="inline mr-2" /> EXPORT TO 3MF</>
                    )}
                </button>
            </div>
